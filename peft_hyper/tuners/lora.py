...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor, task_types=None, return_route_weight = False):
        ...
        if self.disable_adapters:
            ...
            if self.r > 0:
                route_logits = self.lora_route(x)  # shape: same as x but last dim is lora_num
                route_weight = nn.functional.softmax(route_logits, dim=-1, dtype=torch.float32).to(result.dtype)
                lora_intermediate = getattr(self, f"lora_A")(self.lora_dropout(x))  # shape: same as x but last dim is r
                for i in range(self.lora_num):
                    lora_output = getattr(self, f"lora_B{i}")(lora_intermediate)  # shape: same as result
                    branch_weight = route_weight[..., i]
                    branch_weight = branch_weight.unsqueeze(-1)
                    result = result + branch_weight * lora_output * self.scaling
