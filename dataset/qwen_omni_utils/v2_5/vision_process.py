...
def fetch_video(ele: dict, image_factor: int = IMAGE_FACTOR, return_video_sample_fps: bool = False) -> torch.Tensor | list[Image.Image]:
    if isinstance(ele["video"], str):
        ...
        try:
            ...
            try:
                print(f'retrying with nframes=2 for video: {ele["video"]}')
                ele_retry = ele.copy()
                ele_retry['nframes'] = 2
                
                video, sample_fps = VIDEO_READER_BACKENDS[video_reader_backend](ele_retry)
                print(f'successfully read video with nframes=2: {ele["video"]}')
            except Exception as e2:
                print(f'read video error (second attempt with nframes=1). video_path: {ele["video"]} error info: {str(e2)}')
                print(f'returning zero tensor for video: {ele["video"]}')
